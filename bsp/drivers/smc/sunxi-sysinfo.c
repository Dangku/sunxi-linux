		...
		unsigned long ioctl_param)
{
	...
	case CHECK_SOC_SECURE_ATTR:
		...
		if (ret)
			pr_debug("soc is secure. return value: %ld\n", ret);
		else
			pr_debug("soc is normal. return value: %ld\n", ret);
		break;
	case CHECK_SOC_VERSION:
		ret = sunxi_get_soc_ver();
		pr_debug("soc version:%lx\n", ret);
		break;
	...
	case CHECK_SOC_ROTPK_STATUS:
		...
		break;
	case CHECK_SOC_BATCHNO:
		ret = sunxi_get_platform_id();
		pr_debug("soc batchno is:%lx\n", ret);
		break;
	default:
			     ...
			     struct class_attribute *attr, char *buf)
{
	...
	size += sprintf(buf + size, "sunxi_batchno     : %#x\n",
			sunxi_get_platform_id());

	/* soc ver */
	size += sprintf(buf + size, "sunxi_soc_ver    : %#x\n",
			sunxi_get_soc_ver());
